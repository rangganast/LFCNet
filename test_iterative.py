...
class depth_rgb_proj:
    ...
    def __call__(self, point_cloud, rgb_img, T_ext, device, image_path='test_calib.png'):
...
class pcd_extrinsic_transform_realign: # transform PCD into fisheye camera reference frame.
    ...
    def __call__(self, point_cloud, T_ext):
        ...
        pcd_cam = torch.matmul(T_ext.inverse(), point_cloud_h.T).T  # (N, 4)
...
def test(weights, loader, batch_size):
    ...
    # Metrics storage
    ex_epoch = [[] for _ in range(num_stages)]
    ey_epoch = [[] for _ in range(num_stages)]
    ez_epoch = [[] for _ in range(num_stages)]
    et_epoch = [[] for _ in range(num_stages)]
    eyaw_epoch = [[] for _ in range(num_stages)]
    eroll_epoch = [[] for _ in range(num_stages)]
    epitch_epoch = [[] for _ in range(num_stages)]
    er_epoch = [[] for _ in range(num_stages)]
    ...
    gen_rgb_proj_img = depth_rgb_proj(image_size=img_size, K_int=K, distort_params=distort_params)
    ...
    # seeds = random.sample(range(0, 10), 5)
    seeds = random.sample(range(0, len(loader)), 5)
    
    print("Chosen index for generating projection result:", str(seeds))

    with torch.no_grad():
        for i, batch_data in enumerate(tqdm(loader, desc="Iterative Refinement")):
            # Prepare data
            pcd = batch_data["pcd"].squeeze(0).to(DEVICE)
            rgb_img = batch_data["img"].to(DEVICE)
            rgb_img_ori = batch_data["img_ori"].to(DEVICE)
            ...
            if i in seeds:
                y = gen_rgb_proj_img(pcd, rgb_img_ori, T_mis, DEVICE, f"figures/misalignment_init_{i}.png")
                y = gen_rgb_proj_img(pcd, rgb_img_ori, T_velo_to_cam_gt, DEVICE, f"figures/ground_truth_{i}.png")
            ...
            for stage in range(num_stages):
                ...
                ex_epoch[stage].append(e_x.item())
                ey_epoch[stage].append(e_y.item())
                ez_epoch[stage].append(e_z.item())
                et_epoch[stage].append(e_t.item())
                eyaw_epoch[stage].append(e_yaw.item() * 180 / torch.pi)
                eroll_epoch[stage].append(e_roll.item() * 180 / torch.pi)
                epitch_epoch[stage].append(e_pitch.item() * 180 / torch.pi)
                er_epoch[stage].append(e_r.item() * 180 / torch.pi)
            ...
            if i in seeds:
                y = gen_rgb_proj_img(pcd, rgb_img_ori, T_mis, DEVICE, f"figures/calibrated_{i}.png")

    # Print results
    for stage in range(num_stages):
        print(f"== Stage {stage+1} ==")
        print(f"== X AXIS ==")
        print("x-axis error mean:", f"{100 * np.asarray(ex_epoch[stage]).mean():.4f}", "cm")
        print("x-axis error median:", f"{np.median(100 * np.asarray(ex_epoch[stage])):.4f}", "cm")
        print("x-axis error std:", f"{np.std(100 * np.asarray(ex_epoch[stage])):.4f}", "cm")

        print("== Y AXIS ==")
        print("y-axis error mean:", f"{100*np.asarray(ey_epoch[stage]).mean():.4f}", "cm")
        print("y-axis error median:", f"{np.median(100*np.asarray(ey_epoch[stage])):.4f}", "cm")
        print("y-axis error std:", f"{np.std(100*np.asarray(ey_epoch[stage])):.4f}", "cm")

        print("== Z AXIS ==")
        print("z-axis error mean:", f"{100*np.asarray(ez_epoch[stage]).mean():.4f}", "cm")
        print("z-axis error median:", f"{np.median(100*np.asarray(ez_epoch[stage])):.4f}", "cm")
        print("z-axis error std:", f"{np.std(100*np.asarray(ez_epoch[stage])):.4f}", "cm")

        print("== ET ==")
        print("Translation error:", f"{100*np.asarray(et_epoch[stage]).mean():.4f}", "cm")
        ...
        print("Rotation error median:", f"{np.median(np.asarray(er_epoch[stage])):.4f}", f"\N{DEGREE SIGN}")
        print("Rotation error std:", f"{np.std(np.asarray(er_epoch[stage])):.4f}", f"\N{DEGREE SIGN}")
...
if __name__ == "__main__":
    ...
    val_dataset = KITTI360_Fisheye_Dataset(rootdir="/home/rangganast/rangganast/dataset/KITTI-360",
                                        ...
                                        return_pcd=True,
                                        device=DEVICE)
    val_loader = DataLoader(dataset=val_dataset, batch_size=BATCH_SIZE, shuffle=False, pin_memory=False, num_workers=2, drop_last=True)
